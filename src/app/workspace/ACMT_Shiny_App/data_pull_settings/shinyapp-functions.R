...
radiiDF<-data.frame(radius=c(500, 1000, 2000, 5000))
...
ratings_table<-function(dataset){
  ratings.table<-dataset %>%
    mutate(GeocodeRating=ifelse(rating==0, '0', ifelse(rating>0 & rating<10, '0 to 9', ifelse(rating>9 & rating < 21, '10-20', 
                                                                                                ifelse(rating>20, '20 or higher', NA)))))%>%
...
outputDir <- "~/workspace/ACMT_Shiny_App"
...
table_summary<-function(dataset) {
  ...
  if(nrow(dataset)==0){data_summary='No data pulled yet'}
  return(data_summary)
